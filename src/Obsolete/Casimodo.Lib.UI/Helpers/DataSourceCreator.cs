...
{
#if (SILVERLIGHT && RIAS)
    // Code by Vladimir Bodurov.
    // See http://blog.bodurov.com/How-to-bind-Silverlight-DataGrid-from-IEnumerable-of-IDictionary
    // TODO: Ask Vladimir what licence text should go into this file.
    //
    public static class DataSourceCreator
    {
        ...
        public static object ToDataObject(IDictionary dict)
        {
            if (dict == null)
                throw new ArgumentNullException("dict");
            
            var typeSigniture = GetTypeSigniture(dict);
            var type = GetTypeByTypeSigniture(typeSigniture);            
            ...
            if (type == null)
            {
                ...
                {
                    if (PropertNameRegex.IsMatch(Convert.ToString(entry.Key), 0))
                    {
                        CreateProperty(tb, Convert.ToString(entry.Key), GetValueType(entry.Value), onExecMethod);
                    }
                    else
                    {
                ...
                _typeBySigniture.Add(typeSigniture, type);
            }

            return GenerateItem(type, dict);
        }

        static object GenerateItem(Type itemType, IDictionary dict)
        {
            ...
            foreach (DictionaryEntry entry in dict)
            {
                var property = itemType.GetProperty(Convert.ToString(entry.Key));
                property.SetValue(
                    item,
                    Convert.ChangeType(entry.Value, property.PropertyType, null),
                    null);
            }
