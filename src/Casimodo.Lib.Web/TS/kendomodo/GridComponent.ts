...
/// <reference path="../casimodo/Navigation.ts" />
...
namespace kmodo {
    ...
    export function foreachGridRow(grid: kendo.ui.Grid, action: ($row: JQuery, item: any) => void) {
        ...
        grid.tbody.find("tr[role=row]").each((index, elem) => {
            var $row = $(elem);
    ...
    export class GridComponent extends DataSourceViewComponent {
        ...
        */
        ...
        foreachGridRow(action: ($row: JQuery, item: any) => void): void {
            ...
            this.grid().tbody.find("tr[role=row]").each((index, elem) => {
                var $row = $(elem);
        ...
        private _initRowImageCells(item, $row: JQuery): void {
            ...
            $row.find(".kendomodo-button-show-image").each((index, elem) => {
                var $btn = $(elem);
        ...
        */
        ...
        createComponent(): void {
            ...
            if (this._options.baseFilters) {
                ...
                optionFilters.forEach(x => {
                    if (Array.isArray(x))
                        baseFilters.push.apply(baseFilters, x);
                    else
                        baseFilters.push(x);
                });
            ...
            $toolbar.find(".custom-command").on("click", (e) => {
                self._executeCustomCommand($(this).attr("data-command-name"));
            });
            ...
            if (!this._options.isDialog &&
                ...
                $(window).resize(function (e) {
                    ...
                    $grid.find(">div:not(.k-grid-content)").each((index, elem) => {
    ...
    export class GridSelectionManager extends cmodo.ComponentBase {
        ...
        private _getIsEnabled() {
            return this.gridViewModel.getSelectionMode() === "multiple";
        }

        public _onDataSourceDataBound(e) {
            if (!this._getIsEnabled())
                return;
        ...
        private _initSelectors() {
            ...
            try {
                ...
                this.getKendoGrid().tbody.find("tr[role='row']").each((index, elem) => {
                    var $row = $(elem);
        ...
        private _updateSelectedViewStates() {
            var self = this;
            this._iselectors.forEach(function (isel) {
                isel.isSelected = self._getIsSelectedById(isel.id);
                isel.$selector.prop("checked", isel.isSelected);
            });
        }

        _onAllSelectorChanged($selector) {
        ...
        _removeDataItem(item) {
            this._removeDataItemById(item[this.keyName]);
        };

        _removeDataItemById(id: string) {
