...
(function (kendomodo) {
    ...
    kendomodo.openDeletionConfirmationDialog = function (message, title) {
        return kendomodo.openConfirmationDialog(message, title || "Löschen bestätigen");
    };

    kendomodo.openConfirmationDialog = function (message, title) {
        ...
        wnd.content("<div class='confirmation-dialog-content'><div><h5>" +
            message +
            "</h5></div><hr/><div class='confirmation-dialog-actions'>" +
            "<button class='k-button confirmation-ok'></button>" +
            "<button class='k-button confirmation-cancel'></button></div></div>");

        $wnd.find("button.confirmation-ok").text("Ja").on("click", function () {
            result = true;
            deferred.resolve(result);
            wnd.close();
        });

        $wnd.find("button.confirmation-cancel").text("Nein").on("click", function () {
            wnd.close(result);
            deferred.resolve(result);
        });
    ...
    kendomodo.LoadProgressManager = (function () {
    ...
    kendomodo.showModalTextDialog = function (kind, text) {

        if (!kind) {
            text = "<span style='font-size:1.2em'>" + text + "</span>";
        }
        else if (kind === 'info') {
            text = "<span style='font-size:1.2em;color:blue'>" + text + "</span>";
        }
        else if (kind === 'warning') {
            text = "<span style='font-size:1.2em;color:orange'>" + text + "</span>";
        }
        else if (kind === 'error') {
            text = "<span style='font-size:1.2em;color:maroon'>" + text + "</span>";
        }
        ...
        var wnd = $('<div></div>')
            ...
            .data('kendoWindow').content(text).center().open();
    };
