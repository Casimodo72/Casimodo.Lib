...
import { CMatModel, CMatModelErrors, CMatModelLabel } from "@lib/ui/forms/fields"
import { PickerModel, PickerItemModel } from "@lib/models/inputs/pickerModel"
...
@Component({
    selector: "app-standard-typeahead-picker",
    standalone: true,
    imports: [
        CommonModule,
...
@if (model(); as model) {
    <mat-form-field
        ...
        <!-- @if (something) {
        <app-button matSuffix [type]="something.icon" (click)="something.trigger()" />
        } -->
...
export class StandardTypeaheadPickerComponent
