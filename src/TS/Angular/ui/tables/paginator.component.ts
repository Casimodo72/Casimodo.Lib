...
import { StandardPickerComponent } from "@lib/ui/components/standard-picker.component"
import { PaginationModel } from "../../data/dataSource/paginationModel"
...
@if (model(); as model) {
    <div class="w-full flex items-center gap-2" [attr.disabled]="isEffectivelyDisabled()">
        ...
        @if (model.isSizeSelectable()) {
            <app-standard-picker [model]="pageSizePicker" [disabled]="isEffectivelyDisabled()" [groupStyle]="{ 'width': '80px'}"/>
            <div>je Seite</div>
        }
...
export class PaginatorComponent extends FormPartComponent
    implements AfterViewInit {
    readonly pageSizePicker = new PickerModel<number>(this)
